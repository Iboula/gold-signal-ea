...
double DgsHigh(ENUM_TIMEFRAMES tf,int bars,int shift)
{
   ...
   for(int i=shift+1;i<shift+bars;i++) value=MathMax(value,iHigh(_Symbol,tf,i));
   return value;
}
...
double DgsLow(ENUM_TIMEFRAMES tf,int bars,int shift)
{
   ...
   for(int i=shift+1;i<shift+bars;i++) value=MathMin(value,iLow(_Symbol,tf,i));
   return value;
}

bool DgsTrendUp()
{
   return GetEMAValue(PERIOD_M15,20,1)>GetEMAValue(PERIOD_M15,50,1);
}
...
SignalResult AnalyzeDeepScalp(int rangeBars,int bosBars,double sweepPts,double confirmPts,double bufferPts,double tpR,bool useAtrCap,int atrPeriod,double atrMult,double maxStopPts)
{
   ...
   if(DgsTrendUp() && low<rangeLow-sweep && close>rangeLow && close>open && close>bosHigh+confirm)
   {
      ...
      SignalResult s=BuildBuySignal(SymbolInfoDouble(_Symbol,SYMBOL_ASK),sl,atr,useAtrCap,atrMult,maxStopPts,"DeepScalp: sweep low + M1 BOS + M15 trend");
   ...
   if(DgsTrendDown() && high>rangeHigh+sweep && close<rangeHigh && close<open && close<bosLow-confirm)
   {
      ...
      SignalResult s=BuildSellSignal(SymbolInfoDouble(_Symbol,SYMBOL_BID),sl,atr,useAtrCap,atrMult,maxStopPts,"DeepScalp: sweep high + M1 BOS + M15 trend");
